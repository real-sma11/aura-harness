...
//!
//! Codex-derived execution discipline: a "keep going", a "do not
//! guess", a decision / ambiguity / bias-to-act / exit clause. Build
//! / test commands are spliced inline so the agent's mental model
//! matches the DoD gate's invocation; platform info lives in
//! [`super::project_context`].

/// Render the dev-loop workflow block.
///
/// `build_cmd` and `test_cmd` are spliced into the prose verbatim;
/// the caller is responsible for substituting the env override /
/// project fallback / `(not configured)` placeholder before invoking
/// us.
#[must_use]
pub fn render(build_cmd: &str, test_cmd: &str) -> String {
    let body = format!(
...
/// Host-platform notice spliced into the project-context block. Pulled
/// out as a `const fn` so test scaffolding can scrub the line
/// cross-platform and so [`super::project_context`] can reuse it
/// without re-implementing the shell-dispatch matrix.
#[must_use]
pub const fn platform_info_string() -> &'static str {
    if cfg!(windows) {
