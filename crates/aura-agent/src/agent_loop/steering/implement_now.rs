...
//!
//! Phase 5 added [`ImplementNowSteering`]: a [`super::TurnSteering`]
//! wrapper that owns its own exploration / write / read-path
//! telemetry so the trait-driven path can drop the
//! `evaluate_implement_now(config, state)` borrow on `LoopState`.
//! The stateless function remains for the legacy chat path that
//! does not own a [`super::SteeringRegistry`] yet.
...
/// counters rather than borrowing them from [`LoopState`] — keeping
/// every per-source state self-contained per the Phase 5 design
/// goal. `LoopState` still maintains parallel `exploration_state` /
/// `had_any_file_write` fields for other consumers
/// (`tool_pipeline::partition_circling_duplicate_reads`,
/// `compute_thinking_effort`); the two stay in lockstep because both
/// are driven by `tool_pipeline::track_tool_effects` in the same
/// call.
#[derive(Debug)]
pub(crate) struct ImplementNowSteering {
    /// Whether the wrapping [`AgentLoopConfig`] declared a
    /// `phase_reset_signal`. Tracked tasks (the dev-loop / TaskRun
    /// path) set this `true`; chat and generic callers leave it
    /// `false`, in which case the gate never fires.
    ...
    /// True once any successful write tool has been observed. Mirrors
    /// `LoopState::had_any_file_write`.
...
impl ImplementNowSteering {
    /// Construct a new source. `has_phase_reset_signal` mirrors
    /// `AgentLoopConfig::phase_reset_signal.is_some()` — the dev-loop
    /// task path threads a shared `Arc<AtomicBool>` and sets this
    /// `true`; chat / generic callers leave it `false` and the gate
    /// stays permanently disarmed.
    #[must_use]
    pub(crate) fn new(has_phase_reset_signal: bool) -> Self {
        Self {
...
impl TurnSteering for ImplementNowSteering {
    fn observe_tool(&mut self, tool: &ToolCallInfo, result: &ToolCallResult) {
        ...
        if helpers::is_write_tool(&tool.name) && !result.is_error {
            // A path-bearing write OR a multi-file write that
            // attached `file_changes` both count as forward motion.
...
mod tests {
    ...
    fn impl_now_self_contained_fires_at_threshold() {
        ...
        // Second turn: latch holds; no re-fire even though the
    ...
    fn impl_now_self_contained_skipped_after_write() {
        ...
        write_res.file_changes = vec![crate::types::FileChange {
    ...
    fn impl_now_self_contained_includes_sample_paths_sorted() {
        let mut src = ImplementNowSteering::new(true);
        let threshold = aura_config::agent().steering.implement_now_threshold;
        // First two reads have specific paths we'll check the
        // alphabetical ordering of; remaining reads use throwaway
        // paths so we cross the threshold.
