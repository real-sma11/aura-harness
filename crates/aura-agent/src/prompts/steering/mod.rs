...
pub use implement_now_gate::IMPLEMENT_NOW_DEFAULT_THRESHOLD;
// `evaluate_implement_now` is `pub(crate)` because it takes a
// `LoopState` (also `pub(crate)`); the re-export must match. Phase 2
// will relocate the evaluator into `agent_loop/steering/` and revisit.
pub(crate) use implement_now_gate::evaluate_implement_now;
