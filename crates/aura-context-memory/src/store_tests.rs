...
fn sealed_store(dir: &std::path::Path) -> MemoryStore {
    let cipher = Arc::new(aura_store_db::SealCipher::new(&[9u8; 32]));
...
fn sealed_fact_roundtrip_and_ciphertext_on_disk() {
    ...
    let cf = store.db().cf_handle("memory_facts").unwrap();
    let raw = store
        .db()
        ...
        .unwrap()
        .1;
    assert!(aura_store_db::SealCipher::is_sealed(&raw));
    assert!(!raw
...
fn sealed_event_and_procedure_roundtrip() {
    ...
    store.put_event(&make_event(agent, "run", "did a thing", now)).unwrap();
    assert_eq!(store.list_events(agent, 10).unwrap().len(), 1);
    assert_eq!(
        store
            .list_events_since(agent, now - Duration::hours(1))
            .unwrap()
            .len(),
        1
    );

    let proc = make_procedure(agent, "deploy", &["build", "push"]);
    store.put_procedure(&proc).unwrap();
    assert_eq!(
